...
async function GetUser(user){
    try {
        ...
        if(result.rows.length === 0 || result.rows[0] === undefined || result.rows[0] === null){
            return { message: 'Usuario no encontrado', success: false, status: 404 }
        }
        ...
        return { success: true, status: 200, user: result.rows[0] }
...
async function EditUser(user){
    try {
        ...
        const userN = await pool.query(query2, [user.emailNew])
        ...
        return { message: 'Error interno del servidor al editar usuario', success: false, status: 500 };
    }
}
